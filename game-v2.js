...
let prefs={god:false,gentle:false,music:true,sound:true,hd:true,checkpoints:false,tutorialSeen:false},progress={done:[],last:0};
...
function toggleMusic(){prefs.music=!prefs.music;audioStart();if(musicBus){musicBus.gain.cancelScheduledValues(ac.currentTime);musicBus.gain.setTargetAtTime(prefs.music?.12:0,ac.currentTime,.03);nextBeat=ac.currentTime;if(!prefs.music&&window.WillySoundtrack)window.WillySoundtrack.cancelOneShot(ac);}persist();sync();}
function options(paused=false){
 mode=paused?'pause':'settings';
 showModal(`<div class="eyebrow">LITTLE WILLY · SETTINGS</div><h2>${paused?'Pause':'Your Adventure'}</h2><div class="row"><div><strong>God Mode</strong><small>Invincible. Keys and level goals are still required.</small></div><button id="godSetting" class="switch" aria-pressed="${prefs.god}">${prefs.god?'On':'Off'}</button></div><div class="row"><div><strong>Music</strong><small>Full soundtrack with unique themes for every world.</small></div><button id="musicSetting" class="switch" aria-pressed="${prefs.music}">${prefs.music?'On':'Off'}</button></div><div class="row"><div><strong>Sound Effects</strong><small>Shots, jumps, hits and collected items.</small></div><button id="soundSetting" class="switch" aria-pressed="${prefs.sound}">${prefs.sound?'On':'Off'}</button></div><div class="row"><div><strong>Gentler Difficulty</strong><small>Start each level with 6 hearts instead of 4; enemies move more slowly.</small></div><button id="gentleSetting" class="switch" aria-pressed="${prefs.gentle}">${prefs.gentle?'On':'Off'}</button></div><div class="row"><div><strong>Graphics</strong><small>Smoother outlines, new gradients and cartoon Willy.</small></div><button id="hdSetting" class="switch" aria-pressed="${prefs.hd}">${prefs.hd?'HD':'DOS Pixels'}</button></div><div class="row"><div><strong>Checkpoints</strong><small>Save on safe solid ground with ⚑ Save or C. Death restores that point, items and keys. Restart Level clears it. Checkpoints last for this level visit.</small></div><button id="checkpointSetting" class="switch" aria-pressed="${prefs.checkpoints}">${prefs.checkpoints?'On':'Off'}</button></div><div class="instructions">← → / A D: move · Space: jump · S / J: shoot · ↑ / W: door<br>↓: drop through a thin platform · G: God Mode · M: music · Esc: pause</div><div class="panel-actions"><button id="helpSetting" class="secondary">How to Play</button><button id="resume" class="primary">${paused?'Continue':'Back'}</button>${paused?'<button id="restart" class="secondary">Restart Level</button><button id="hub" class="secondary">Space Station</button><button id="menu" class="secondary">Main Menu</button>':''}</div>`);
 $('checkpointSetting').onclick=()=>{prefs.checkpoints=!prefs.checkpoints;game.setCheckpoints(prefs.checkpoints);persist();options(paused);};$('godSetting').onclick=()=>{toggleGod();options(paused);};$('musicSetting').onclick=()=>{toggleMusic();options(paused);};$('soundSetting').onclick=()=>{prefs.sound=!prefs.sound;audioStart();persist();options(paused);};$('gentleSetting').onclick=()=>{prefs.gentle=!prefs.gentle;game.setGentle(prefs.gentle);persist();options(paused);};$('hdSetting').onclick=()=>{prefs.hd=!prefs.hd;WillyRenderer.setHD(prefs.hd);persist();options(paused);};$('resume').onclick=resume;$('helpSetting').onclick=()=>help();
...
function help(step=0){
 const pages=[['Welcome to Earth II','Start with World 02: The Ice Caves. You can also explore the space station and choose any unlocked door.'],['Move, jump and shoot','Use the on-screen arrows, JUMP and SHOOT. Keyboard: ← → or A/D to move, Space to jump, S/J to shoot. Press DOOR ↑ or W near a station door.'],['Unlock the exit','Find the exit card AND collect the required cans or lollipops. The counter at the top shows what is missing. Coloured keys open matching locks. World 20 is special: do not collect any cans!'],['Your pace, your adventure','Checkpoints are optional in Settings. Turn them on, then press ⚑ Save or C on safe solid ground. They restore your items and keys after a death. God Mode is also available.']];
...
function finish(){
 mode='complete';progress.done=game.done;progress.last=0;persist();
...
function frame(now){
 ...
 if(mode==='play'){
  acc+=elapsed;while(acc>=1/120){game.step(1/120,{...keys,jump:jumpPress,up:upPress,shoot:keys.shoot||shootPress});jumpPress=false;upPress=false;shootPress=false;acc-=1/120;
   const events=game.events.splice(0);for(const e of events){sound(e.type);if(e.type==='enter'){start(e.index);break;}if(e.type==='complete'){finish();break;}if(e.type==='exitInfo')toast(exitMessage(e));if(e.type==='card')toast('Exit card found · '+exitMessage(game.exitStatus()));if(e.type==='unlock')toast('Lock opened');if(e.type==='lockedFinale')toast('The final level unlocks after the other 23 levels.');if(e.type==='respawn')toast(e.checkpoint?'Back at your checkpoint · Items and keys restored':'New attempt · The level starts over.');if(e.type==='forbidden')toast('Do not collect cans here! Returning to your last safe start.');}
